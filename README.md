...
The CSI driver has been tested with XtremIO X2 XIOS 6.2.0 and 6.2.1.

### Operating Systems Supported
* CentOS 7.6 with Linux kernel 3.10+
* Ubuntu and other distributions may be supported, but were not extensively tested.
...
2. One of the k8s nodes must be designated to be the `master` or `controller`. It needs to run **kubectl** as part of controller initialization. From here on, this node will also be referred to as `controller`. All other nodes will be referenced as `nodes`. Clone this repository to the `controller`:
```bash
...
5. On `controller` node run the following command:
```bash
$ ./install-csi-plugin.sh -c
```
6. On all other nodes, run the following command:
```bash
...
| csi_username | The CSI user account to be used by the plugin in all REST calls to the XIO System. This user can be created in advance by a user or by the script. In the latter case, the initial username and password with Tech role are required. | Yes |
...
| Name | Explanation |
|--------------|------------------|
|    `-c`    |      Controller initialization, should run once only on one of k8s nodes. This option creates a user account, QoS policies, if necessary, and loads YAML definitions for the driver into k8s cluster. It also configures the Initiator Group of this node in the XMS.
|    `-n`    |      Node initialization, should run on all other nodes. Configures an Initiator Group of the node is in the XMS.
...
|    `-u`    |    Update the plugin from the new yaml files. With this flag, the installation script updates the docker images and YAML definitions for the driver. **USE WITH CARE!** See the explanation in the Upgrade section (below).
